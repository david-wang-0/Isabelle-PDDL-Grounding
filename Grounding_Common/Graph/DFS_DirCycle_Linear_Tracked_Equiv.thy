...
text \<open>Level 1 of the refinement chain --- the \<^emph>\<open>tracked\<close> directed-cycle search of
  \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux\<close> / \<open>DFS_DirCycle_Tracked\<close> --- against level 0,
  the reference implementation of \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Aux\<close> /
