...
text \<open>Level 2 of the refinement chain at the \<^emph>\<open>inner\<close> (pre-seeded) directed-cycle DFS: the
  ...
  \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux\<close> becomes once the adjacency map moves into the
  state.
...
text \<open>The callbacks of \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux\<close> verbatim, except that
...
text \<open>Level 2's own equivalence, from \<^theory>\<open>Grounding_Graph.DFS_Skel_More_Refine\<close>: the loop
...
text \<open>The capstone of this theory: forgetting the carried graph, the refined run \<^emph>\<open>is\<close> the
  tracked run of \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux\<close>. Everything proved there ---
