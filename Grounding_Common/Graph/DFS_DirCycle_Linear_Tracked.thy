...
text \<open>The whole-graph (linear) directed-cycle search of
  ...
  \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux\<close>. The algorithm is the same sweep --- pick a
  ...
  \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux\<close>.
...
text \<open>\<^theory>\<open>Grounding_Graph.DFS_Skel_More\<close> declares \<open>termination_intros\<close> \<^emph>\<open>inside\<close>
...
text \<open>What the inner DFS must deliver. The first six conjuncts are those of
  ...
  \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux\<close> (\<open>dircycle_tracked_finished_inv\<close>,
