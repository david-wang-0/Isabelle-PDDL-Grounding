...
text \<open>Level 2 of the refinement chain at the \<^emph>\<open>outer\<close> (whole-graph) sweep --- the companion of
  \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux_Refine\<close>, which refines the inner
  ...
  \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux_Refine\<close>. Level 1's state predicates
  ...
  \<^theory>\<open>Grounding_Graph.DFS_DirCycle_Linear_Tracked_Aux_Refine\<close>, and note that (a) is equality of the
