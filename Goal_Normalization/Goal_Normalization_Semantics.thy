...
text ‹ Proving valid_classical_plan2_right ›

lemma resinst_goal_ac:
  "(the o p3.res_inst) π⇩g = GroundAction (goal P) goal_effect"
proof -
  have "goal_ac term_goal ∈ set (actions D3)" using degoal_dom_sel by simp
  hence "(the o p3.res_inst) π⇩g = GroundAction
...
lemma g_goal_sem_right:
  assumes "wf_world_model M" "valuation M ⊨⇩m goal P" 
  shows
    "p3.plan_action_enabled π⇩g M"
    "valuation (p3.execute_plan_action π⇩g M) ⊨⇩m goal P3"
proof -
  from assms show "p3.plan_action_enabled π⇩g M"
  ...
  from assms(1) this
  have basic: "lwm_basic (fst (p3.execute_plan_action π⇩g M))"
    using g_wm_wf p3_wf.wf_execute_stronger wf_goal_pa p3.wf_lwm_basic
          p3.plan_action_enabled_def
    by metis

  from resinst_goal_ac have "effect ((the o p3.res_inst) π⇩g) = goal_effect"
    by simp
  hence "Atom (predAtm goal_pred []) ∈ fst (p3.execute_plan_action π⇩g M)"
    using p3.execute_plan_action_def by simp
  hence "valuation (p3.execute_plan_action π⇩g M) (predAtm goal_pred []) = Some True"
    using valuation_def by simp
  with basic show "valuation (p3.execute_plan_action π⇩g M) ⊨⇩m goal P3"
    by simp
qed
...
(* TODO clean up *)
lemma g_wf_pa_right:
  assumes "wf_classical_plan_action π"
  shows
    "(the o res_inst) π = (the o p3.res_inst) π" "p3.wf_classical_plan_action π"
proof -
  obtain n args where
  ...
  have acD3: "ac ∈ set (actions D3)" using ac by simp
  ...
  show g1: "(the o res_inst) π = (the o p3.res_inst) π"
    using res_inst_alt p3.res_inst_alt using res res3 by simp

  show wf3: "p3.wf_classical_plan_action π"
...
lemma g_exec_right:
  assumes "wf_classical_plan_action π"
  shows "execute_plan_action π M = p3.execute_plan_action π M"
  using assms ast_classical_problem.execute_plan_action_def g_wf_pa_right by simp

lemma g_enab_right:
  assumes "wf_classical_plan_action π"
  shows "plan_action_enabled π M = p3.plan_action_enabled π M"
  using assms g_wf_pa_right ast_classical_problem.plan_action_enabled_def by simp

lemma g_execs_right:
  assumes "wf_world_model M" "valid_classical_plan_alt M πs M'"
  shows "p3.valid_classical_plan_alt M πs M'"
  using assms proof (induction πs arbitrary: M)
case (Cons p ps)
...
proof -
  from assms obtain M where 1: "valid_classical_plan_alt I πs M" and 2: "valuation M ⊨⇩m goal P"
    using valid_classical_plan2_alt by auto

  from 1 have wf_m: "wf_world_model M"
    using wf_I wf_valid_classical_plan_alt by blast
  from 1 have "p3.valid_classical_plan_alt p3.I πs M"
    using ast_classical_problem.I_def g_execs_right wf_I by auto
  moreover from 2 wf_m have "p3.plan_action_enabled π⇩g M"
           "valuation (p3.execute_plan_action π⇩g M) ⊨⇩m goal P3"
...
lemma g_ac_split: "ac ∈ set (actions D3) ⟷ ac ∈ set (actions D) ∨ ac = goal_ac term_goal"
  using degoal_dom_sel by auto

lemma iffOrI: "⟦⟦A; ¬C⟧ ⟹ B; B ⟹ A; C ⟹ A⟧ ⟹ A ⟷ B ∨ C"
  by blast

lemma g_wf_pa_left:
  "p3.wf_classical_plan_action π ⟷ wf_classical_plan_action π ∨ π = π⇩g"
proof (cases π)
  case [simp]: (SimplePlanAction n args)
  show ?thesis proof (rule iffOrI)
    assume 1: "p3.wf_classical_plan_action π" and 2: "¬π = π⇩g"
    from 1 obtain ac where 3:
      "p3.resolve_classical_action_schema n = Some ac" "ac ∈ set (actions D3)"
      "ac_name ac = n" "p3.action_params_match (head ac) args"
      using p3.wf_pa_refs_ac by auto

    have "ac ≠ goal_ac term_goal"
    proof (rule eq_contr)
      ...
      ultimately have "π = π⇩g" using 3 by simp
      with 2 show False by simp
    qed

    hence "ac ∈ set (actions D)" using 3 g_ac_split by simp
    ...
    ultimately show "wf_classical_plan_action π"
  ...
  next
    assume "wf_classical_plan_action π" thus "p3.wf_classical_plan_action π"
      using g_wf_pa_right by blast
  next
    assume "π = π⇩g" thus "p3.wf_classical_plan_action π"
...
lemma goal_atm_not_wf: "¬wf_fmla_atom tyt (Atom (predAtm goal_pred []))"
proof -
  have "safe_prefix pred_names + STR ''Goal'' ∉ set pred_names"
    using safe_prefix_correct by blast
  hence "safe_prefix pred_names + STR ''Goal'' ∉ predicate.name ` (pred ` set (predicates D))"
    by auto
  hence "goal_pred ∉ pred ` set (predicates D)"
    unfolding goal_pred_def by (metis image_iff predicate.sel)
  hence "¬wf_pred_atom tyt (goal_pred, [])"
    using sig_None wf_pred_atom.simps by (metis option.simps(4))
  thus ?thesis by simp
qed

lemma g_goal_unaffected:
  assumes "¬valuation M ⊨⇩m goal P3" "wf_classical_plan_action π" "p3.wf_world_model M"
  shows "¬valuation (p3.execute_plan_action π M) ⊨⇩m goal P3"
proof -
  let ?goal_atm = "Atom (predAtm goal_pred [])"

  let ?ga = "(the o p3.res_inst) π"
  ...
  hence "∀ae ∈ set (adds (effect ?ga)). wf_fmla_atom objT ae"
    using wf_effect_alt unfolding list_all_iff by blast
  hence notin: "?goal_atm ∉ set (adds (effect ?ga))"
    using goal_atm_not_wf by blast

  have end_basic: "lwm_basic (fst (p3.execute_plan_action π M))"
    using assms g_wf_pa_right p3_wf.wf_execute_stronger p3.wf_lwm_basic by blast

  (* Todo simplify / export *)
  from assms have "¬ valuation M ⊨⇩m goal P3"
    using p3.wf_lwm_basic by blast
  hence "valuation M (predAtm goal_pred []) ≠ Some True"
    by simp
  hence "?goal_atm ∉ fst M" using valuation_def by simp
  with notin have "?goal_atm ∉ fst (p3.execute_plan_action π M)"
    using p3.execute_plan_action_def apply_ground_action_alt by simp
  hence "valuation (p3.execute_plan_action π M) (predAtm goal_pred []) ≠ Some True"
    using valuation_def by simp
  hence "¬ valuation (p3.execute_plan_action π M) ⊨⇩m goal P3"
    by simp
  thus ?thesis using assms end_basic by blast
qed

lemma g_init_not_sat: "¬ valuation p3.I ⊨⇩m goal P3"
proof -
  have "(Atom (predAtm goal_pred [])) ∉ fst I"
    using wf_I goal_atm_not_wf wf_lwm_basic lwm_basic_def by fastforce
  hence "valuation p3.I (predAtm goal_pred []) ≠ Some True"
    using valuation_def by simp
  thus ?thesis using p3_wf.i_basic by simp
qed

lemma no_goal_pa_no_sat:
  assumes "p3.wf_world_model M" "¬valuation M ⊨⇩m goal P3" "π⇩g ∉ set πs" "p3.valid_classical_plan_alt M πs M'"
  shows "¬valuation M' ⊨⇩m goal P3"
using assms proof (induction πs arbitrary: M)
  case Nil then show ?case
    using p3.valid_classical_plan2_def p3.valid_classical_plan_from2_def by auto
next
  case (Cons π πs)
  hence "p3.wf_classical_plan_action π"
    using p3.valid_classical_plan_alt.simps(2) p3.plan_action_enabled_def by blast
  hence "wf_classical_plan_action π" using g_wf_pa_left ‹π⇩g ∉ set (π # πs)› by auto

  from Cons have 1: "p3.wf_world_model (p3.execute_plan_action π M)"
    using p3_wf.wf_execute_stronger ‹p3.wf_classical_plan_action π› by simp
  from Cons have 2: "¬valuation (p3.execute_plan_action π M) ⊨⇩m goal P3"
    using g_goal_unaffected ‹wf_classical_plan_action π› by blast
  from Cons have 3: "π⇩g ∉ set πs" by simp
  from Cons have 4: "p3.valid_classical_plan_alt (p3.execute_plan_action π M) πs M'"
...
lemma no_goal_pa_not_valid:
  assumes "π⇩g ∉ set πs"
  shows "¬p3.valid_classical_plan2 πs"
proof -
  have "p3.valid_classical_plan2 πs ⟷ (∃M. p3.valid_classical_plan_alt p3.I πs M ∧ valuation M ⊨⇩m goal P3)"
    unfolding p3.valid_classical_plan2_def p3.valid_classical_plan_from2_alt by simp
  (* with no_goal_pa_no_sat we get
    p3.valid_classical_plan_alt p3.I πs M ⟹ ¬valuation M ⊨⇩m goal P3 *)
  with assms show ?thesis using g_init_not_sat no_goal_pa_no_sat p3_wf.wf_I by blast
qed

lemma g_valid_classical_plan2_has_ga:
  "p3.valid_classical_plan2 πs ⟹ π⇩g ∈ set πs"
  using no_goal_pa_not_valid by auto

lemma g_goal_sem_left:
  assumes "p3.plan_action_enabled π⇩g M"
  shows "valuation M ⊨⇩m goal P"
using assms p3.plan_action_enabled_def wf_goal_pa
    resinst_goal_ac by auto

lemma g_exec_left:
  assumes "p3.wf_classical_plan_action π" "π ≠ π⇩g"
  shows "p3.execute_plan_action π M = execute_plan_action π M"
  using assms g_wf_pa_left g_exec_right by simp

lemma g_enab_left:
  assumes "p3.wf_classical_plan_action π" "π ≠ π⇩g"
  shows "p3.plan_action_enabled π M = plan_action_enabled π M"
  using assms g_wf_pa_left g_enab_right by simp

lemma g_execs_left:
  assumes "wf_world_model M" "π⇩g ∉ set πs" "p3.valid_classical_plan_alt M πs M'"
  shows "valid_classical_plan_alt M πs M'"
using assms proof (induction πs arbitrary: M)
  case (Cons π πs)
  hence neq: "π ≠ π⇩g" and notin: "π⇩g ∉ set πs" by auto
  from Cons have enab3: "p3.plan_action_enabled π M" and path3: "p3.valid_classical_plan_alt (p3.execute_plan_action π M) πs M'"
    by simp_all

  from enab3 have wf3: "p3.wf_classical_plan_action π" using p3.plan_action_enabled_def by simp
  with enab3 have enab: "plan_action_enabled π M" using neq g_enab_left by simp
  from path3 wf3 have path2: "p3.valid_classical_plan_alt (execute_plan_action π M) πs M'" 
    using neq g_exec_left by simp

  from Cons enab have wf_m: "wf_world_model (execute_plan_action π M)" 
    using wf_execute_stronger plan_action_enabled_def by blast

  have path: "valid_classical_plan_alt (execute_plan_action π M) πs M'" 
...
proof -
  let ?ogs = "restore_plan_degoal πs"
  from assms obtain ys where
    restore: "(?ogs @ [π⇩g]) @ ys = πs" (* parantheses important for unification with valid_classical_plan_alt_append_elim *)
    using g_valid_classical_plan2_has_ga sublist_just_until by fastforce
  from assms obtain M3 where
    fullpath: "p3.valid_classical_plan_alt p3.I πs M3"
    using p3.valid_classical_plan2_alt by blast
  then obtain M2 where
    path: "p3.valid_classical_plan_alt p3.I (?ogs @ [π⇩g]) M2"
  ...
  then obtain M1 where
    ...
    skip: "p3.valid_classical_plan_alt M1 [π⇩g] M2"
    using p3.valid_classical_plan_alt_append_elim by blast

  from skip have "p3.plan_action_enabled π⇩g M1"
    using valid_classical_plan_alt.simps(2) by simp
  hence sat: "valuation M1 ⊨⇩m goal P"
