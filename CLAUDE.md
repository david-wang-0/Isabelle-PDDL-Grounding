...
Once per session, before any other `mcp__isabelle__*` call, authenticate by passing the value of the `IQ_AUTH_TOKEN` environment variable to `mcp__isabelle__authenticate` (read it via `printf '%s' "$IQ_AUTH_TOKEN"`; the literal `$IQ_AUTH_TOKEN` is not expanded inside the token argument).
