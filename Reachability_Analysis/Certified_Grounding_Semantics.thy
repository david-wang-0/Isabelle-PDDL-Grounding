...
section \<open>Certificate-to-grounder bridge (Option 1): the \<open>wf_grounder\<close> plug-in at \<open>P\<close>\<close>

text \<open>Step 4: interpret \<^locale>\<open>wf_grounder\<close> at the \<^emph>\<open>un-relaxed\<close> problem \<open>P\<close>, with the
  certificate-derived facts/ops. The semantic supersets are \<open>all_facts_super\<close>/\<open>all_ops_super\<close>;
  the syntactic well-formedness/coverage obligations are re-checked by \<open>grounding_checks\<close> and
  discharged below. This is the point that retires \<open>wf_grounder_args\<close>'s sorry in
  \<open>Grounding_Pipeline\<close>.\<close>
...
text \<open>Each decidable grounder obligation is read off the \<open>grounding_cert\<close> assumption (the kernel's
  re-check of the certificate). Phrasings match the \<^locale>\<open>wf_grounder\<close> goals exactly.\<close>
...
proof unfold_locales
  ...
  show "distinct cert_ops'" by (simp add: cert_ops'_def cert_ops_of_def px.cert_ops_def)
