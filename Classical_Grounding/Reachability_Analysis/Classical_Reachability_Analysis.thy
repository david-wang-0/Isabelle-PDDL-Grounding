theory Classical_Reachability_Analysis
  imports Classical_Grounding_Utils.Classical_PDDL_Sema_Supplement
...
text \<open>This theory keeps only the \<^emph>\<open>shared\<close> PDDL \<rightarrow> datalog infrastructure that the certificate
  development (\<open>Classical_PDDL_Reachability_Locales\<close> / \<open>Classical_PDDL_Reachability_Analysis\<close>) builds on: the
