...
subsection \<open>Linear orders on \<^type>\<open>predicate\<close> and \<^type>\<open>object\<close> (for the RBT certificate index)\<close>

text \<open>The verified fact-driven closure check (\<^const>\<open>dl_closure_check_exec_fast\<close>) indexes the
  certificate facts in a red-black tree, which needs \<^class>\<open>linorder\<close> keys. Both \<^type>\<open>predicate\<close>
  and \<^type>\<open>object\<close> wrap a \<^typ>\<open>String.literal\<close> (\<open>name\<close>), which is already a \<^class>\<open>linorder\<close>
  in the distribution, so we lift that order through the single constructor. This is a plain
  \<^class>\<open>linorder\<close> instance (no Containers/\<open>ceq\<close>/\<open>ccompare\<close>), keeping the datalog layer
