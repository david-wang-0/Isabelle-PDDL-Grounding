theory Classical_Goal_Normalization
  imports Classical_Goal_Normalization_Locales
begin

section \<open>Goal Normalization Well-Formedness Proofs\<close>

context wf_ast_classical_problem3 begin

text \<open>The goal predicate is fresh.\<close>
...
proof -
  have "wf_function_decl fd \<Longrightarrow> p3.wf_function_decl fd" for fd
...
lemma g_consts_wf: "\<forall>(n, T) \<in> set (consts D3). p3.wf_type T"
...
proof -
  have in_preds: "(p, xs) \<in> set (map split_pred (predicates D3))"
  proof -
...
lemma g_fmla_wf: "wf_fmla tyt \<phi> \<Longrightarrow> p3.wf_fmla tyt \<phi>"
...
lemma g_eff_wf: "wf_effect tyt \<epsilon> \<Longrightarrow> p3.wf_effect tyt \<epsilon>"
...
lemma g_fmla_atom_wf: "wf_fmla_atom tyt f \<Longrightarrow> p3.wf_fmla_atom tyt f"
  using g_atom_wf co_fmla_atom_wf by blast

lemma g_func_assign_wf: "wf_func_assign f \<Longrightarrow> p3.wf_func_assign f"
...
lemma g_action_head_wf: "wf_action_head h \<Longrightarrow> p3.wf_action_head h"
  by (cases h) simp

lemma g_simple_action_body_wf:
  assumes "tyt \<subseteq>\<^sub>m tyt3" and "wf_simple_action_body tyt b"
...
lemma g_acs_wf: "(\<forall>a\<in>set (actions D3). p3.wf_classical_action_schema a)"
proof -
...
text \<open>The degoaled domain and problem are well-formed.\<close>
...
lemma g_init_wf:
  "\<forall>f\<in>set (init P3). p3.wf_fmla_atom p3.objT f \<or> p3.wf_func_assign f"
proof
  fix f assume "f \<in> set (init P3)"
  hence "f \<in> set (init P)" by simp
  with wf_P(4) have "wf_fmla_atom objT f \<or> wf_func_assign f" by blast
  thus "p3.wf_fmla_atom p3.objT f \<or> p3.wf_func_assign f"
...
sublocale wf_ast_classical_problem3 \<subseteq> p3_wf: wf_ast_classical_domain D3
  using degoal_dom_wf wf_ast_classical_domain.intro by simp

sublocale wf_ast_classical_problem3 \<subseteq> p3_wf: wf_ast_classical_problem P3
...
text \<open>The goal action resolves in the degoaled domain.\<close>
...
lemma wf_goal_pa:
  "p3.wf_classical_plan_action \<pi>\<^sub>g"
...
text \<open>Mapping a closed formula through \<open>term.CONST\<close> and back is the identity.\<close>
...
proof -
  have "map_atom (ac_tsubst [] []) (map_atom term.CONST a)
      = map_atom (\<lambda>x. ac_tsubst [] [] (term.CONST x)) a"
