...
lemma varfree_inst_action_map_entry:
  assumes "\<pi> \<in> set ops"
  obtains n where
    "(\<pi>, n) \<in> set (zip ops op_names)"
    "varfree_inst_ac \<pi> n \<in> set (actions varfree_inst_dom)"
...
proof -
  ...
  have ntheq: "actions varfree_inst_dom ! i = varfree_inst_ac \<pi> ?n"
    using i ops_len by simp
  have li: "i < length (actions varfree_inst_dom)" using i ops_len by simp
  have ac: "varfree_inst_ac \<pi> ?n \<in> set (actions varfree_inst_dom)"
...
lemma resinst_varfree_inst_pa:
  assumes "\<pi> \<in> set ops"
  shows "png.res_inst (ground_pa \<pi>) = res_inst \<pi>"
proof -
  from assms obtain n where n:
    "varfree_inst_ac \<pi> n \<in> set (actions varfree_inst_dom)" "op_map_inv \<pi> = Some n"
    using varfree_inst_action_map_entry by metis
  have nm: "name (ground_pa \<pi>) = n" unfolding ground_pa_def using n(2) by simp
  have nname: "ast_classical_action_schema_name (varfree_inst_ac x m) = m" for x m
    by (simp add: varfree_inst_ac_def)
  have res: "png.resolve_classical_action_schema n = Some (varfree_inst_ac \<pi> n)"
    unfolding png.resolve_classical_action_schema_def
    using n(1) varfree_inst_dom_names_dis nname
    by (simp add: ast_classical_domain.resolve_classical_action_schema_def index_by_eq_Some_eq)
  have "png.res_inst (ground_pa \<pi>) = Some (instantiate_classical_action_schema (varfree_inst_ac \<pi> n) [])"
    unfolding png.res_inst_alt using res nm by (simp add: ground_pa_def)
  also have "\<dots> = res_inst \<pi>" using varfree_inst_ac_inst by (simp add: res_inst_alt)
  finally show ?thesis .
qed
...
proof -
  ...
  show ?thesis
    unfolding gp using res pm by (simp add: varfree_inst_ac_def)
qed
...
lemma varfree_inst_enabled_iff:
  assumes "\<pi> \<in> set ops"
  shows "plan_action_enabled \<pi> M \<longleftrightarrow> png.plan_action_enabled (ground_pa \<pi>) M"
proof -
  have ri: "png.res_inst (ground_pa \<pi>) = res_inst \<pi>" using resinst_varfree_inst_pa[OF assms] .
  have wfl: "wf_classical_plan_action \<pi>" using assms ops_wf by blast
  have wfr: "png.wf_classical_plan_action (ground_pa \<pi>)" using varfree_inst_pa_wf[OF assms] .
  show ?thesis
...
next
  ...
  have ex: "png.execute_plan_action (ground_pa \<pi>) M = execute_plan_action \<pi> M"
    using varfree_inst_exec_right[OF pi] .
  have "png.plan_action_enabled (ground_pa \<pi>) M"
    using varfree_inst_enabled_iff[OF pi] en by simp
...
proof -
  ...
  have path: "png.valid_classical_plan_alt png.I (map ground_pa \<pi>s) M'"
    unfolding varfree_png_init using varfree_inst_path_right[OF ops M'(1)] .
  have goal: "valuation M' \<Turnstile>\<^sub>m goal varfree_inst_prob" using M'(2) by simp
...
text \<open>Every action schema in the numeric grounded domain is nullary, so a well-formed grounded
  ...
  derive nullarity directly from \<^const>\<open>varfree_inst_ac\<close>).\<close>
lemma varfree_inst_pa_nullary:
...
proof -
  ...
  have "index_by ast_classical_action_schema_name (actions varfree_inst_dom) n = Some a"
    using a(1) unfolding png.resolve_classical_action_schema_def by simp
  hence "a \<in> set (actions varfree_inst_dom) \<and> ast_classical_action_schema_name a = n"
    by (rule index_by_eq_SomeD)
  hence amem: "a \<in> set (actions varfree_inst_dom)"
    and an: "ast_classical_action_schema_name a = n" by simp_all
  from amem obtain \<pi>'' m where am: "a = varfree_inst_ac \<pi>'' m"
    unfolding varfree_inst_dom_sel using map2_obtain by metis
  have hd: "head a = ActionHead m []" unfolding am by (simp add: varfree_inst_ac_def)
  have "m = n" using an unfolding am by (simp add: varfree_inst_ac_def)
  ...
  show "n \<in> set op_names"
    using an amem varfree_inst_dom_names by (metis image_eqI list.set_map)
qed
...
proof -
  ...
  with assms have nin: "n \<in> set op_names" and args: "args = []"
    using varfree_inst_pa_nullary by blast+
...
next
  ...
  have enM: "plan_action_enabled ?\<pi> M"
    using varfree_inst_enabled_iff[OF piops] en g by simp
  have ex: "png.execute_plan_action \<pi>' M = execute_plan_action ?\<pi> M"
    using varfree_inst_exec_right[OF piops] g by simp
...
proof -
  from assms obtain M' where g: "png.valid_classical_plan_alt png.I \<pi>s' M'"
    "valuation M' \<Turnstile>\<^sub>m goal varfree_inst_prob" using png.valid_classical_plan2_alt by blast
  have path: "valid_classical_plan_alt I (map restore_ground_pa \<pi>s') M'"
    using varfree_inst_path_left[OF g(1)[unfolded varfree_png_init]] .
...
text \<open>Every fact achievable in the numeric grounded problem is achievable in \<open>P\<close>: a
  ...
  (\<open>varfree_inst_path_left\<close>), so the achieving world model is shared.\<close>
...
proof -
  ...
  have "valid_classical_plan_alt I (map restore_ground_pa \<pi>s') M"
    using varfree_inst_path_left[OF M[unfolded varfree_png_init]] .
